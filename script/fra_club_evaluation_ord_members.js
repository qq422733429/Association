 ...
 * 函数功能:展示某社团中普通社员某一年度的考核信息(此函数需要用用户在社团中的社团和职位，以及年度考核信息)
 * 函数参数: userID--用户ID,
 * 		   societyID--社团ID
 * 		   yearStr--哪一年度
 * 返回值:回调函数执行结果
 * */
function showOrdUserEvaInfo(userID, societyID, yearStr){
	api.showProgress({
	    style: 'default',
	    animationType: 'fade',
	    title: '努力加载考核信息中...',
 ...
 * 作者:lzm
 * 编写日期:2015-8-19 10:38
 * 函数名:showOrdUserEvaInfoEx2
 * 函数功能:展示某社团中普通社员某一年度的考核信息(此函数在index.js调用了common_club_evaluation.js中updateSocietyMemYearEvaEx,updateSocietyYearEva后直接查询数据库展示)
 * 函数参数: userID:用户ID
 * 		  societyID：所在社团ID
 * 		  yearStr:所在年度形式'2015'
 * 返回值:回调函数执行
 * */
function showOrdUserEvaInfoEx2(userID, societyID, yearStr){
	api.showProgress({
	    style: 'default',
	    animationType: 'fade',
	    title: '努力加载中...',
 ...
 * 作者:lzm
 * 编写日期:2015-8-19 10:38
 * 函数名:showOrdUserEvaInfoEx3
 * 函数功能:展示某社团中普通社员某一年度的考核信息(此函数所有展示数据都取自本地，减少用户等待时间)
 * 函数参数: userID:用户ID
 * 		  societyID：所在社团ID
 * 		  yearStr:所在年度形式'2015'
 * 返回值:回调函数执行
 * */
function showOrdUserEvaInfoEx3(userID, societyID, yearStr){
	api.showProgress({
	    style: 'default',
	    animationType: 'fade',
	    title: '努力加载中...',
...
//	alert( "localStorage societyID:" + societyID + ",userID:" + userID );
	...
	showOrdUserEvaInfoEx3(userID, societyID, yearStr);
//	//显示社团考核信息,这里有多层回调，记得顺序
//	updateSocietyMemYearEvaEx(societyID, userID, yearStr, showOrdUserEvaInfoEx);
//	//触发更新社团年度考核信息
